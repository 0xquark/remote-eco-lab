...
params:
  ...
  cpuBaseline: !r data.frame(0)
  cpuMeasurement: !r data.frame(0)
  cpuMeasurementActions: !r data.frame(0)
  allCpuMeasurements: !r data.frame(0)
  allCpuBaselines: !r data.frame(0)
  ...
  tooShortActions: NULL
  cpucolnames: !r c("a","b")
  cpucolnamesbaseline: !r c("a","b")
---
...
# Introduction
This report contains the analysis results from the Artificial Intelligence IoT (AIoT) resource consumption measurement "`r params$measurementName`". The measurements were executed $n_M = `r params$measurementCount`$ times. The baseline was averaged over $n_B = `r params$baselineCount`$ passes. SUT was \textit{`r params$SUT`}.
...
par("mar" = c(20.1, 4.1, 4.1, 2.1), "cex.axis" = 2.2, "cex.main" = 2.5, "cex.lab" = 2.5)
x <- boxplot(actionsTable, main = "Boxplots of the action durations", xaxt = "n", space = 1)
text(cex=1, x=c(1:length(actionsTable))-0.3, y=-500, substring(names(actionsTable), 1, 20), xpd=TRUE, srt=45)
dev.off()
rm(x)
```
...
To evaluate the energy consumption that is caused by the software product, we first calculate the mean energy consumption [Watt-hours] of the scenario and the baseline measurement as $W_{el} = P\cdot{}t$. Because the measurements of the power usage $P_n$ were recorded with a sampling rate of $F=1 Hz$, the energy can be calculated as
\begin{equation}
\label{eqn:Wel}
W_{el} = \frac{1}{3600} \sum_{n=1}^{m} P_n, \text{ mit } P_n=P_1, P_2, P_3, ..., P_m
\end{equation}
Thus, the energy for one testrun or baseline equals the sum of the per-second measurement values (resulting in the unit Watt-seconds). It is then divided by $3600\frac{s}{h}$, resulting in the unit Watt-hours. We then average the energy consumption means over the testruns and baselines, respectively. The results can be found in table \ref{tab:energyResults}.
...
This section contains the results for the energy consumption analysis of the individual actions. The results shown in table \ref{tab:ActionResultsPower} represent the mean values for the power and energy consumption of the actions, calculated in the same way as the testrun results. If the measurement duration of an action is too short for a reliable analysis (i.~e. less than one second), this is denoted as "too short". The Scenario induced energy is plotted in figure \ref{fig:powerWhAction}.
...
for(action in actionNames){
  if(action %in% tooShortActions){
    resultTableActions <- rbind(resultTableActions, c(0, 0, 0, 0))
  } else {
...
actionNamesForTable <- actionNames
actionNamesForTable[which(actionNames %in% tooShortActions)] <- paste0(actionNamesForTable[which(actionNames %in% tooShortActions)], "_too short")
...
if(!is.null(params$modelHistory)){
  cat("\\clearpage\n\n# Model history\n\n")
  cat("This section describes the data from the ML process. Table \\ref{tab:history_means} and figure \\ref{fig:accuracy_means} show the results.\n\n")
  print(kable(params$modelHistory, caption="Model score over all testruns\\label{tab:history_means}", digits = 3))
  cat("\n\n![Boxplot of the model score\\label{fig:accuracy_means}](", params$graphicsFolder, "/model_history/model_history_boxplot.png)\n\n", sep = "")
}
```
...
# Hardware Usage
\label{sec:hardware}
The analysis of the hardware utilization is, for the most part, analogous to the power analysis. The data is averaged over time for each indicator. Table \ref{tab:hwIndicators} shows which parameters were logged (using collectl, cf. the commands in _Scenarios/baseline.sh_ and _Scenarios/scenarios.sh_) and analyzed.
...
disk activity (reading and writing)      & kB   &  [DSK]ReadKBTot + [DSK]WriteKBTot \\
Network traffic (sending und receiving) & kB   &  [NET]RxKBTot + [NET]TxKBTot \\ \hline
\multicolumn{3}{l}{\textbf{Individual CPU cores}} \\
Indicator      & Unit & Name in collectl \\ \hline
```{r results='asis', echo = FALSE}
for(core in params$cpucolnames){
  cat(paste0(toupper(gsub("_", " ", core)), " & \\% & [", toupper(gsub("_", ":", core)), "]Totl\\% \\\\ \n"))
}
```
```{r results='asis', echo = FALSE}
...
disk activity & figure \ref{fig:hw_hdd} & figure \ref{fig:hw_hdd_bl} \\
Network traffic & figure \ref{fig:hw_nw} & figure \ref{fig:hw_nw_bl} \\
CPU core loads overview & \multicolumn{2}{l}{figure \ref{fig:hw_cores_all_in_one}}\\
```{r results='asis', echo = FALSE}
for(core in params$cpucolnames){
  cat(paste0("Load of ", gsub("_", " core ", toupper(core)), " & figure \\ref{fig:hw_", core, "} & figure \\ref{fig:hw_", core, "_bl} \\\\ \n"))
}
```
```{r results='asis', echo = FALSE}
if(evaluateGpuMeasurement){
...
![Graph of the processor load\label{fig:hw_cpu}](`r paste(params$graphicsFolder, "cpu_usage_total.png", sep="/")`)
...
![Graph of the disk activity\label{fig:hw_hdd}](`r paste(params$graphicsFolder, "hdd_activity.png", sep="/")`)

![Graph of the baseline disk activity\label{fig:hw_hdd_bl}](`r paste(params$graphicsFolder, "hdd_activity_baseline.png", sep="/")`)
...
![Overview graph of the CPU cores\label{fig:hw_cores_all_in_one}](`r paste(params$graphicsFolder, "cpu_per_core.png", sep="/")`)

```{r results='asis', echo = FALSE}
for(core in params$cpucolnames){
  cat(paste0("![Graph of processor load of ", gsub("_", " core ", toupper(core)), "\\label{fig:hw_", core, "}](", params$graphicsFolder, "/", core, "_usage.png) \n\n"))
  cat(paste0("![Graph of baseline processor load of ", gsub("_", " core ", toupper(core)), "\\label{fig:hw_", core, "_bl}](", params$graphicsFolder, "/", core, "_usage_baseline.png) \n\n"))
}
...
This section contains the descriptive analysis of each parameter. The mean usage that is induced by the scenario $\overline{x}(Parameter_{S})$ is calculated as the mean value of the measurement testruns $\overline{x}(Parameter_{T})$ from which the mean value of the baseline $\overline{x}(Parameter_{B})$ is subtracted. Thus, the average value is calculated as
$$\overline{x}(Parameter_{S}) = \overline{x}(Parameter_{T}) - \overline{x}(Parameter_{B})\text{.}$$
...
cpuCoresMeansTables <- list()
cpuCoreMeansPerTestrun <- list()
i <- 1
for(core in params$cpucolnames){
  coreMeans <- NULL
  for(j in 1:length(params$cpuMeasurement)){
    coreMeans[j] <- mean(unlist(params$cpuMeasurement[[j]][core]))
  }
  cpuCoresMeansTables[[i]] <- describe(coreMeans, IQR = T)
  cpuCoreMeansPerTestrun[[i]] <- coreMeans
  i <- i + 1
}
...
cpuCoresMeansBaselineTables <- list()
cpuCoreMeansPerBaseline <- list()
i <- 1
for(core in params$cpucolnames){
  coreMeansBaseline <- NULL
  for(j in 1:length(params$cpuBaseline)){
    coreMeansBaseline[j] <- mean(unlist(params$cpuBaseline[[j]][core]))
  }
  cpuCoresMeansBaselineTables[[i]] <- describe(coreMeansBaseline, IQR = T)
  cpuCoreMeansPerBaseline[[i]] <- coreMeansBaseline
  i <- i + 1
}
...
i <- 1
for(core in cpuCoresMeansTables){
  overviewTable <- rbind(overviewTable, core[, c("n","mean", "sd", "median", "min", "max", "range", "IQR")])
  rownames(overviewTable)[nrow(overviewTable)] <- paste0(toupper(gsub("_", " ", cpucolnames[i])), " load [%]")
  i <- i + 1
}

i <- 1
for(core in cpuCoresMeansBaselineTables){
  overviewTable <- rbind(overviewTable, core[, c("n","mean", "sd", "median", "min", "max", "range", "IQR")])
  rownames(overviewTable)[nrow(overviewTable)] <- paste0(toupper(gsub("_", " ", cpucolnamesbaseline[i])), " baseline [%]")
  i <- i + 1
}
...
meanspercore <- do.call(rbind.data.frame, cpuCoresMeansTables)$mean
meanspercoreBaseline <- do.call(rbind.data.frame, cpuCoresMeansBaselineTables)$mean

resultTable <- as.data.frame(cbind(c(mean(cpuMeans), mean(ramMeans), mean(hddMeans), mean(nwMeans), meanspercore),
  c(mean(cpuMeansBaseline), mean(ramMeansBaseline), mean(hddMeansBaseline), mean(nwMeansBaseline), meanspercoreBaseline),
  c(mean(cpuMeans) - mean(cpuMeansBaseline), mean(ramMeans) - mean(ramMeansBaseline), mean(hddMeans) - mean(hddMeansBaseline), mean(nwMeans) - mean(nwMeansBaseline), meanspercore - meanspercoreBaseline)))

resultTable <- cbind(c("CPU load [%]", "RAM usage [GB]", "disk activity [kB]", "Network traffic [kB]", paste0(toupper(gsub("_", " ", cpucolnames)), " load [%]")), resultTable)
...
disk activity & table \ref{tab:hw_hdd} & figure \ref{fig:hw_hdd_overview} & figure \ref{fig:hw_hdd_boxplots}\\
Network traffic & table \ref{tab:hw_nw} & figure \ref{fig:hw_nw_overview} & figure \ref{fig:hw_nw_boxplots}\\
```{r results='asis', echo = FALSE}
for(core in params$cpucolnames){
  cat(paste0("Load of ", gsub("_", " core ", toupper(core)), " & table \\ref{tab:hw_", core, "} & figure \\ref{fig:hw_", core, "_overview} & figure \\ref{fig:hw_", core, "_boxplots} \\\\ \n"))
}
if(evaluateGpuMeasurement){
...
names(resultTable)<-c("testrun #", "mean CPU load [%]","Scenario induced CPU load [%]")
kable(resultTable, caption="individual CPU measurement results\\label{tab:hw_cpu}", digits = 3)
```

```{r echo=FALSE, fig.cap="Plot of the Scenario induced CPU load per measurement\\label{fig:hw_cpu_overview}"}
...
```{r echo=FALSE, fig.cap="Boxplots of the CPU load of the testruns (l) and Scenario induced load (r) \\label{fig:hw_cpu_boxplots}"}
...
boxplot(cpuMeans - mean(cpuMeansBaseline), main = paste("Scenario induced"), ylab="CPU load [%]")
par(opar)
```
...
names(resultTable)<-c("testrun #", "mean RAM usage [%]","Scenario induced RAM usage [%]")
kable(resultTable, caption="individual RAM measurement results\\label{tab:hw_ram}", digits = 3)
```

```{r echo=FALSE, fig.cap="Plot of the Scenario induced RAM usage per measurement\\label{fig:hw_ram_overview}"}
...
```{r echo=FALSE, fig.cap="Boxplots of the RAM usage of the testruns (l) and Scenario induced load (r) \\label{fig:hw_ram_boxplots}"}
opar <- par(mfrow=c(1, 3))
par("mar" = par("mar")+1, "cex.axis" = 1.7, "cex.main" = 1.7, "cex.lab" = 1.7)
boxplot(ramMeansBaseline, main = paste("baselines"), ylab="RAM usage [%]")
boxplot(ramMeans, main = paste("testruns"), ylab="RAM usage [%]")
boxplot(ramMeans - mean(ramMeansBaseline), main = paste("Scenario induced"), ylab="RAM usage [%]")
par(opar)
```
...
names(resultTable)<-c("testrun #", "mean disk activity [kB]","Scenario induced disk activity [kB]")
kable(resultTable, caption="individual disk measurement results\\label{tab:hw_hdd}", digits = 3)
```

```{r echo=FALSE, fig.cap="Plot of the Scenario induced disk activity per measurement\\label{fig:hw_hdd_overview}"}
plot(hddMeans - mean(hddMeansBaseline), type = "h", main = paste("Overview of the disk activity for the measurements"), xlab = "testrun #", ylab = "disk activity [kB]")
```

```{r echo=FALSE, fig.cap="Boxplots of the disk activity of the testruns (l) and Scenario induced load (r) \\label{fig:hw_hdd_boxplots}"}
opar <- par(mfrow=c(1, 3))
par("mar" = par("mar")+1, "cex.axis" = 1.7, "cex.main" = 1.7, "cex.lab" = 1.7)
boxplot(hddMeansBaseline, main = paste("baselines"), ylab="disk activity [kB]")
boxplot(hddMeans, main = paste("testruns"), ylab="disk activity [kB]")
boxplot(hddMeans - mean(hddMeansBaseline), main = paste("Scenario induced"), ylab="disk activity [kB]")
par(opar)
```
...
names(resultTable)<-c("testrun #", "mean network traffic [kB]","Scenario induced network traffic [kB]")
kable(resultTable, caption="individual network measurement results\\label{tab:hw_nw}", digits = 3)
```

```{r echo=FALSE, fig.cap="Plot of the Scenario induced network traffic per measurement\\label{fig:hw_nw_overview}"}
...
```{r echo=FALSE, fig.cap="Boxplots of the network traffic of the testruns (l) and Scenario induced load (r) \\label{fig:hw_nw_boxplots}"}
opar <- par(mfrow=c(1, 3))
par("mar" = par("mar")+1, "cex.axis" = 1.7, "cex.main" = 1.7, "cex.lab" = 1.7)
...
boxplot(nwMeans - mean(nwMeansBaseline), main = paste("Scenario induced"), ylab="network traffic [kB]")
par(opar)
```
...
### CPU per core
```{r cpu-plots, dev='png', fig.show='hide', echo=FALSE}
# generate plots first
i <- 1
for(core in params$cpucolnames){
  plot(cpuCoreMeansPerTestrun[[i]] - mean(meanspercoreBaseline[[i]]), type = "h", main = paste0("Overview of the ", toupper(gsub("_", " ", core)), " load for the measurements"), xlab = "testrun #", ylab = "CPU core load [%]")
  i <- i + 1
}
```

```{r cpu-boxplots, dev='png', fig.show='hide', echo=FALSE}
# generate boxplots first
i <- 1
for(core in params$cpucolnames){
  opar <- par(mfrow=c(1, 3))
  boxplot(meanspercoreBaseline[[i]], main = paste("Baselines"), ylab="CPU core load [%]")
  boxplot(cpuCoreMeansPerBaseline[[i]], main = paste("testruns"), ylab="CPU core load [%]")
  boxplot(cpuCoreMeansPerTestrun[[i]] - meanspercoreBaseline[[i]], main = paste("Scenario induced"), ylab="CPU core load [%]")
  par(opar)
...
```{r, echo=FALSE, results='asis'}
# then put them in the pdf with the captions
i <- 1
for(core in params$cpucolnames){
  cat(paste0("#### ", toupper(gsub("_", " ", core)), "\n\n"))
  
  resultTable <- as.data.frame(list(c(1:length(cpuCoreMeansPerTestrun[[i]]), "mean"), c(cpuCoreMeansPerTestrun[[i]], mean(cpuCoreMeansPerTestrun[[i]])), c(cpuCoreMeansPerTestrun[[i]] - meanspercoreBaseline[i], mean(cpuCoreMeansPerTestrun[[i]]) - meanspercoreBaseline[i])))
  names(resultTable)<-c("testrun #", paste0("mean ", toupper(gsub("_", " ", core)), " load [%]"),paste0("Scenario induced ", toupper(gsub("_", " ", core)), " load [%]"))
  print(kable(resultTable, caption=paste0("individual CPU measurement results\\label{tab:hw_", core, "}"), digits = 3))

  cat(paste0("![Plot of the Scenario induced ", toupper(gsub("_", " ", core)), " load per measurement\\label{fig:hw_", core, "_overview}](", fig_chunk(label = "cpu-plots", ext = "png", number = i), ")\n\n"))
  cat(paste0("![Boxplots of the ", toupper(gsub("_", " ", core)), " load of the testruns (l) and Scenario induced load (r) \\label{fig:hw_", core, "_boxplots}](", fig_chunk(label = "cpu-boxplots", ext = "png", number = i + 1), ")\n\n"))
...
if(!is.null(params$tooShortActions)){
  cat("However, the duration of the following actions was too short to calculate reasonable results. Details below.\n\n")
  cat(paste("*", params$tooShortActions, collapse ="\n"), "\n\n")
}
...
cpuMeansAction <- NULL
for(i in 1:length(params$cpuMeasurementActions)){
  coreMeansAction <- NULL
  for(core in params$cpucolnames){
    coreMeansAction <- c(coreMeansAction, mean(unlist(params$cpuMeasurementActions[[i]][core]) , na.rm = T))
  }
  cpuMeansAction <- rbind(cpuMeansAction, coreMeansAction)
}

cpuMeansAction <- as.data.frame(cpuMeansAction)
names(cpuMeansAction) <- params$cpucolnames

performanceMeansAction$ram <- performanceMeansAction$ram/1024/1024

for(action in actionNames){
  cat("###", action, "\n\n")
  if(action %in% params$tooShortActions){
    cat(paste0("The mean duration of action \"", action, "\" is ", round(actionsDF[which(actionsDF$`Action name` == action),"mean"], digits = 3), " seconds. This is too short to calculate reasonable results. This action will be omitted.\n\n"))
  } else{
    resultTableActions <- c(
    ...
    for(core in params$cpucolnames){
      resultTableActions <- c(resultTableActions, mean(cpuMeansAction[which(startActions$text == action), core], na.rm = T))
    }
    if(evaluateGpuMeasurement){
      resultTableActions <- c(resultTableActions,
    ...
    if(evaluateGpuMeasurement){
      baselineRT <- c(mean(cpuMeansBaseline), mean(ramMeansBaseline), mean(hddMeansBaseline), mean(nwMeansBaseline), meanspercoreBaseline, mean(temperatureMeansBaseline), mean(gpuMeansBaseline), mean(memUtilMeansBaseline), mean(memMibMeansBaseline), mean(smClockMeansBaseline), mean(memClockMeansBaseline), mean(gpuClockMeansBaseline))
      inducedRT <- resultTableActions - c(mean(cpuMeansBaseline), mean(ramMeansBaseline), mean(hddMeansBaseline), mean(nwMeansBaseline), meanspercoreBaseline, mean(temperatureMeansBaseline), mean(gpuMeansBaseline), mean(memUtilMeansBaseline), mean(memMibMeansBaseline), mean(smClockMeansBaseline), mean(memClockMeansBaseline), mean(gpuClockMeansBaseline))
    } else {
      baselineRT <- c(mean(cpuMeansBaseline), mean(ramMeansBaseline), mean(hddMeansBaseline), mean(nwMeansBaseline), meanspercoreBaseline)
      inducedRT <- resultTableActions - c(mean(cpuMeansBaseline), mean(ramMeansBaseline), mean(hddMeansBaseline), mean(nwMeansBaseline), meanspercoreBaseline)
    }
    ...
    if(evaluateGpuMeasurement){
      rownames(resultTableActions) <- c("CPU load [%]", "RAM usage [GB]", "disk activity [kB]", "Network traffic [kB]", paste0(toupper(gsub("_", " ", params$cpucolnames)), " load [%]"), "GPU temperature [°C]", "GPU load [%]", "GPU memory load [%]", "GPU memory allocation [MiB]", "SM clock frequency [MHz]", "GPU memory clock frequency [MHz]", "GPU clock frequency [MHz]")
    } else {
      rownames(resultTableActions) <- c("CPU load [%]", "RAM usage [GB]", "disk activity [kB]", "Network traffic [kB]", paste0(toupper(gsub("_", " ", params$cpucolnames)), " load [%]"))
    }
