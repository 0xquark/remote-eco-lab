<!--
# ========================================================== #
# TEXT BELOW IS THE SAME FOR BOTH SUS AND IDLE MODE SCENARIO #
# ========================================================== #
-->
...
## Durations
Table \ref{tab:measurement_durations} describes the measurement and baseline durations (measured in seconds) and Table \ref{tab:action_durations} describes the duration for the actions. Additionally, Figure \ref{fig:durations} and Figure \ref{fig:action_durations} show the according boxplots.
...
if(!is.null(params$modelHistory)){
  cat("\\clearpage\n\n# Model history\n\n")
  cat("This section describes the data from the ML process. It includes training and validation loss and accuracy of the models from each testrun and for all epochs (Table \\ref{tab:history_means_epoch} and Table \\ref{tab:history_means} and Figure \\ref{fig:loss_means} and Figure \\ref{fig:accuracy_means}).\n\n")
...
# Hardware Usage
\label{sec:hardware}
The analysis of the hardware utilization is, for the most part, analogous to the power analysis. The data is averaged over time and for power measurements, the energy is calculated. Table \ref{tab:hwIndicators} shows which parameters were logged (using the command `collectl -s cdmn -i1 -P --sep 59 -f`) and analyzed.
...
Table \ref{tab:hardwareOverview} and Table \ref{tab:hardwareResults} list the resulting means for each parameter and baseline.
...
if(evaluateGpuMeasurement){
  cat("Additionally, the indicators \"GPU Performance state\" (pstate), \"Current PCI-E link generation\" (pcie.link.gen.current) are described in a bar plot (Figure \\ref{fig:gpu_barplots}).\n\n")
...
if(evaluateGpuMeasurement){
  ...
  cat("For the GPU power draw (power.draw), logged with nvidia-smi, the energy consumption is also calculated according to formulas \\ref{eqn:Wel} to \\ref{eqn:Wel_train}. The results are displayed in Table \\ref{tab:gpuEnergyResults} (overview) and Table \\ref{tab:gpuTestrunResults} (individual testruns).\n\n", sep = "")
...
```{r echo=FALSE, fig.cap="Boxplots of the CPU load of the testruns (l) and induced load (r) \\label{fig:hw_cpu_boxplots}"}
...
```{r echo=FALSE, fig.cap="Boxplots of the RAM usage of the testruns (l) and induced load (r) \\label{fig:hw_ram_boxplots}"}
opar <- par(mfrow=c(1, 3))
par("mar" = par("mar")+1, "cex.axis" = 1.7, "cex.main" = 1.7, "cex.lab" = 1.7)
...
names(resultTable)<-c("testrun #", "mean HDD activity [kB]","Scenario induced HDD activity [kB]")
kable(resultTable, caption="individual HDD measurement results\\label{tab:hw_hdd}", digits = 3)
```

```{r echo=FALSE, fig.cap="Plot of the scenario induced HDD activity per measurement\\label{fig:hw_hdd_overview}"}
...
```{r echo=FALSE, fig.cap="Boxplots of the HDD activity of the testruns (l) and induced load (r) \\label{fig:hw_hdd_boxplots}"}
opar <- par(mfrow=c(1, 3))
par("mar" = par("mar")+1, "cex.axis" = 1.7, "cex.main" = 1.7, "cex.lab" = 1.7)
...
boxplot(hddMeans - mean(hddMeansBaseline), main = paste("Scenario induced"), ylab="HDD activity [kB]")
par(opar)
```
...
```{r echo=FALSE, fig.cap="Boxplots of the network traffic of the testruns (l) and induced load (r) \\label{fig:hw_nw_boxplots}"}
opar <- par(mfrow=c(1, 3))
par("mar" = par("mar")+1, "cex.axis" = 1.7, "cex.main" = 1.7, "cex.lab" = 1.7)
...
cat(paste0("The results shown in the tables below represent the mean values for the hardware ", gpuText, "of the actions, calculated in the same way as the testrun results.\n\n"))
